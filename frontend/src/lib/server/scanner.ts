...
async function listR2Keys(): Promise<string[]> {
  ...
  do {
    const res = await client.send(new ListObjectsV2Command({ Bucket: env.R2_BUCKET, ContinuationToken: continuationToken }));
...
function scanAmbiences(keys: string[]): { categories: AmbienceCategory[]; ambiences: Ambience[] } {
...
function scanPlaylists(keys: string[]): { categories: PlaylistCategory[]; playlists: Playlist[] } {
  const cats = new Set<string>();
  const plData = new Map<string, { src: string; thumb_src: string | null; tracks: MusicTrack[] }>();
  ...
  for (const key of keys) {
    ...
    if (!plData.has(plKey)) plData.set(plKey, { src: '', thumb_src: null, tracks: [] });

    const pl = plData.get(plKey)!;
    if (filename === 'cover.webp') pl.src = key;
    else if (filename === 'cover.thumb.webp') pl.thumb_src = key;
    else if (filename.endsWith('.ogg')) pl.tracks.push({ id: filename.slice(0, -4), src: key });
  }

  for (const pl of plData.values()) pl.tracks.sort((a, b) => a.src.localeCompare(b.src));
...
function scanScenes(): { categories: SceneCategory[]; scenes: Scene[] } {
  ...
  for (const [path, mod] of Object.entries(sceneModules)) {
    try {
      ...
      const sceneId = (raw.id as string | undefined) ?? path.split('/').pop()!.replace('.json', '');
      ...
      const layers: LayerAsset[] = ((raw.layers as unknown[] | undefined) ?? []).map((lr, i) => {
      ...
      scenes.push({ id: sceneId, label: (raw.label as string | undefined) ?? toLabel(sceneId), background, layers });
      if (!catScenes.has(category)) catScenes.set(category, []);
      catScenes.get(category)!.push({ id: sceneId, label: scenes.at(-1)!.label });
    } catch (err) {
  ...
  const categories: SceneCategory[] = [...catScenes.entries()]
    .sort((a, b) => toOrder(a[0]) - toOrder(b[0]))
    .map(([cat, entries]) => ({ id: cat, label: toLabel(cat), order: toOrder(cat), scenes: entries }));
